...
function failed_report_summary(report: HostedTestReport | undefined) {
  ...
  return {
    ...
    failingSuites: report.suiteRuns.filter((suite) => suite.status !== "pass").map((suite) => ({
      id: suite.id, status: suite.status, errors: suite.errors,
