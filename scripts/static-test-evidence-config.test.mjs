...
async function accepted_fixture() {
  ...
  await mkdir(join(root, "site", evidenceRoot), { recursive: true });
  await writeFile(acceptancePath, JSON.stringify({ accepted: true, evidenceRoot, artifactSet: "b".repeat(64) }));
  await writeFile(join(root, "site", evidenceRoot, "index.json"), JSON.stringify({ deployment: { hsonDeployCommit: commit } }));
