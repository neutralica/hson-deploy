...
const syncTargets = [
	{
		name: "hson-live",
		canonicalPath: path.resolve(deployRoot, "..", "hson-live"),
	},
	{
		name: "hson-demo2",
		canonicalPath: path.resolve(deployRoot, "..", "hson-demo2"),
	},
	{ name: "intrastructure" },
];
...
function assertMainUpstream(repoPath, label) {
	...
	if (branch !== "main") {
		fail(
			`${label} must be on main; currently on ${branch || "(detached HEAD)"}.`,
		);
	}
...
function parentGitlink(repoName) {
	const output = git(deployRoot, ["ls-tree", "HEAD", "--", repoName], true);
...
async function synchronizePublishedSubmodules() {
  ...
  // CHANGED: complete all validation/fetch/planning before mutating any submodule.
  for (const target of syncTargets) {
    ...
    plans.push({
      ...
      // CHANGED: retain both revisions so sync can preview old → new.
      currentCommit: deploymentHead,
      publishedCommit,
    });
  }

  // CHANGED: summarize the complete plan before changing any checkout.
  console.log("\nSubmodule sync plan:\n");

  for (const plan of plans) {
    if (plan.behind === 0) {
      console.log(
        `  ${plan.name}: current at ${plan.currentCommit.slice(0, 7)}`,
      );
    ...
    console.log(
      `  ${plan.name}: ${plan.currentCommit.slice(0, 7)} → ` +
        `${plan.publishedCommit.slice(0, 7)} ` +
        `(${plan.behind} commit(s))`,
    );
  }
  ...
  // CHANGED: a no-op sync exits without asking for confirmation.
  if (pendingPlans.length === 0) {
    console.log(
      "\nAll deployment gitlinks already match published origin/main.",
    );
  ...
  // CHANGED: nothing below this point executes until explicitly confirmed.
  if (
    !(await confirm(
      "Advance and stage these submodule gitlinks?",
    ))
  ) {
    fail("Aborted before modifying submodules.");
  }

  const changedGitlinks = [];

  // CHANGED: mutation begins only after the complete plan was validated
  // and explicitly approved.
  for (const plan of pendingPlans) {
    console.log(
      `${plan.name}: advancing ${plan.behind} commit(s) ` +
        `to ${plan.publishedCommit}.`,
    );

    git(
      plan.deploymentPath,
      ["switch", "--detach", plan.publishedCommit],
    );
  ...
  if (
    ...
    staged.some(
      (file) => !changedGitlinks.includes(file),
    )
  ) {
    fail(
  ...
  console.log(
    `Staged parent gitlinks: ${changedGitlinks.join(", ")}`,
  );
  console.log(
...
async function confirm(question) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    // CHANGED: this helper is now shared by update and sync modes.
    fail(
      "Submodule operations require an interactive terminal for confirmation.",
    );
  }
  ...
  try {
    const answer = (
      await interface_.question(`${question} [y/N] `)
    )
      .trim()
      .toLowerCase();
...
async function commitCanonical(canonicalPath, repoName, commitMessage) {
...
function updateDeploymentSubmodule(deploymentPath, repoName, canonicalCommit) {
	...
	const actualCommit = git(deploymentPath, ["rev-parse", "HEAD"], true);
...
async function main() {
	...
	if (targetName === "sync") {
		...
		// CHANGED: sync now asks for confirmation before mutating submodules.
		await synchronizePublishedSubmodules();
		return;
	}
	 ...
	 */
	synchronizeCleanCanonical(canonicalPath, `canonical ${repoName}`);
	...
	updateDeploymentSubmodule(deploymentPath, repoName, canonicalCommit);
...
main().catch((error) => {
	const command =
		process.argv[2] === "sync" ? "submodules:sync" : "submodule:update";
