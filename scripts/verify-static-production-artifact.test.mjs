...
async function fixture(panelSource = `const root="${evidenceRoot}";const marker="data-frozen-panel-state frozen-test-panel";`) {
  ...
  await mkdir(join(work, "site", evidenceRoot.slice(1)), { recursive: true });
  ...
  await writeFile(join(work, "site", evidenceRoot.slice(1), "index.json"), JSON.stringify({ deployment: { hsonDeployCommit: commit }, suites: [] }));
  await mkdir(join(artifact, evidenceRoot.slice(1)), { recursive: true });
  await writeFile(join(artifact, evidenceRoot.slice(1), "index.json"), JSON.stringify({ deployment: { hsonDeployCommit: commit }, suites: [] }));
  ...
  return { artifact, environment: { VITE_TEST_EVIDENCE_ROOT: evidenceRoot, TEST_EVIDENCE_ACCEPTANCE_FILE: accepted } };
}

test("artifact verifier proves exact root embedding and frozen acquisition isolation", async () => {
  const valid = await fixture();
  assert.deepEqual(verify_static_production_artifact(valid), { evidenceRoot, frozenPanelSources: 1, rowArtifacts: 0, rowBytes: 0 });
